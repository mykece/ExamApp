...
public class TrainerClassroomService : ITrainerClassroomService
{
    ...
    {
        var trainerClassRoom = await _trainerClassroomRepository.GetAllAsync(x => x.ClassroomId == classroomAddTrainerDto.TrainerId);

        var toBeDeletedTrainers = trainerClassRoom.Where(x => !classroomAddTrainerDto.SelectedTClassroomIds.Contains(x.TrainerId)).ToList();

        var newTrainers = classroomAddTrainerDto.SelectedTClassroomIds
    .Where(trainerId => !trainerClassRoom.Any(tc => tc.TrainerId == trainerId))
    .Select(trainerId => new TrainerClassroom
    {
        ClassroomId = classroomAddTrainerDto.TrainerId,
        TrainerId = trainerId,
        AssignedDate = DateTime.Now
    })
    .ToList();
    ...
    /// <summary>
    /// İlgili sınıfın seçilen eğitmenini siler.
    /// </summary>
    /// <param name="trainerClassroomDeleteDto"></param>
    /// <returns></returns>
    public async Task<IResult> DeleteTrainerToClassroom(TrainerClassroomDeleteDto trainerClassroomDeleteDto)
    {
        try
        {
            var trainerClassroom = await _trainerClassroomRepository.GetAsync(tc => tc.TrainerId == trainerClassroomDeleteDto.TrainerId && tc.ClassroomId == trainerClassroomDeleteDto.ClassroomId);
