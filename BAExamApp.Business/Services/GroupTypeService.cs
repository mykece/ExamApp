...
public class GroupTypeService : IGroupTypeService
{
    ...
    public async Task<IDataResult<GroupTypeDto>> GetByIdWithClassroomsAsync(Guid id)
    {
        var groupTypes = await _groupTypeRepository.GetAllAsync(
            x => x.Id == id, 
            tracking: true, 
            includes: x => x.Include(g => g.Classrooms) 
        );

        var groupType = groupTypes.FirstOrDefault();
