...
public class ExamService : IExamService
{
    ...
    /// <summary>
    /// Sınava giren her sınıf için, sınavı tamamlayan öğrencilerin tamamlama sürelerinin ortalamasını hesaplar.
    /// </summary>
    /// <param name="examId">İlgili sınavın Id'si.</param>
    /// <returns></returns>
    public async Task<IDataResult<List<ExamClassroomAverageDto>>> GetExamCompletionAveragesByClassAsync(Guid examId)
    {
        var examClassrooms = await _examClassroomsRepository.GetAllAsync(ec => ec.ExamId == examId);

        if (!examClassrooms.Any())
        {
            return new ErrorDataResult<List<ExamClassroomAverageDto>>(Messages.ExamClassroomNotFound);
        }

        var averages = examClassrooms
       .GroupBy(ec => ec.ClassroomId)
       .Select(g => new ExamClassroomAverageDto
       {
           ClassroomId = g.Key,
           ClassroomName = g.FirstOrDefault()?.Classroom?.Name,
           AverageCompletionTime = TimeSpan.FromMinutes(
                g.SelectMany(ec => ec.Exam?.StudentExams ?? new List<StudentExam>())
                 .Where(se => se.StartTime.HasValue && se.EndTime.HasValue)
                 .Select(se => (se.EndTime.Value - se.StartTime.Value).TotalMinutes)
                 .DefaultIfEmpty(0)
                 .Average()
            )
       }).ToList();

        return new SuccessDataResult<List<ExamClassroomAverageDto>>(averages, Messages.ListedSuccess);
    }
}
