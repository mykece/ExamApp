...
async function getApiUser(apiUserId) {

    return $.ajax({
        url: '/admin/apiuser/getapiuser',
        data: { apiUserId: apiUserId },
    });
}

async function loadAdminData(apiUserId) {
    const form = document.getElementById('kt_modal_update_user_form');
    apiUser = await getApiUser(apiUserId);
    console.log(apiUser);
    let profilePhoto = document.getElementById("profilePhoto");
    form.elements["Id"].value = apiUser.id;
    if (apiUser.originalImage !== null && apiUser.originalImage.length > 0) {
        profilePhoto.style.backgroundImage = 'url("data:image/jpeg;base64,' + apiUser.originalImage + '")';
    } else {
        profilePhoto.style.backgroundImage = 'url("/images/blank-profile-picture.png")';
    }
    form.elements["Image"].value = apiUser.originalImage;
    form.elements["FirstName"].value = apiUser.firstName;
    form.elements["LastName"].value = apiUser.lastName;
    form.elements["Gender"].value = apiUser.gender;
    form.elements["Email"].value = apiUser.email;
